...
type Topic interface {
	IsTopicAlreadyCreated(topic TopicName) (exists bool, err error)
	GetAllTopics() ([]TopicName, error)
	CreateTopic(tp TopicName, pc PartitionCount, rc ReplicaCount) (err error)
	DeleteTopic(tp TopicName) (err error)
}
...
func New(ver Version, brk BrokerAddresses, options ...func(*Kafka) error) (*Kafka, error) {
	...
	if k.topic, err = topic.New(ver, brk); err != nil {
...
func (k *Kafka) AppendLog(txID string, data string) error {
	...
	if !topicExists {
		err = k.topic.CreateTopic(txID, k.pc, k.rc)
		if err != nil {
...
func (k *Kafka) Lookup(txID string) ([]string, error) {
	...
	for _, partition := range partitionList {
		wg.Add(1)
		go consumePartition(k.consumer, txID, partition, msgs, errs, k.dur, &wg)
	}
...
func consumePartition(con sarama.Consumer, topic string, partition int32,
